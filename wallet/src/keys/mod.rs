...
/// ```
///
/// Types that don't internally encode the [`NetworkKind`] in which they are valid need some extra
/// steps to override the set of valid network kinds, otherwise only the network kind specified in
/// the [`Xpriv`] or [`Xpub`] will be considered valid.
///
/// ```
/// use bdk_wallet::bitcoin::{bip32, NetworkKind};
/// use bdk_wallet::keys::{
...
///     fn into_extended_key(self) -> Result<ExtendedKey<Ctx>, KeyError> {
///         let xprv = bip32::Xpriv {
///             network: NetworkKind::Main, // pick an arbitrary network kind
///             depth: 0,
///             parent_fingerprint: bip32::Fingerprint::default(),
///             private_key: self.key_data.inner,
///             chain_code: bip32::ChainCode::from(&self.chain_code),
...
///
///         // Override the set of valid network kinds here
///         Ok(descriptor_key.override_valid_network_kinds(any_network_kind()))
///     }
/// }
/// ```
///
